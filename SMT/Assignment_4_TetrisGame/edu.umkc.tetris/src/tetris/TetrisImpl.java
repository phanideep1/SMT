...
import sidepanel.SidePanelImpl;
...
public class TetrisImpl extends JFrame implements ITetrisImp
{
	/**
	 * 
	 */
...
//	private BoardPanelImp board;
	...
	public static TetrisImpl tetris;
	...
	public static TetrisImpl getTetrisImpl(){
		return tetris;
	}
	...
	*/	
	public void load()
	{
		 ...
		 */
		//this._arch.OUT_IBoardType = new BoardPanel(this);
		this.side = new SidePanelImpl(this);
		
		/*
		 * Add the BoardPanel and SidePanel instances to the window.
		 */
		add(_arch.OUT_IBoardType.boardpanelobject(), BorderLayout.CENTER);
		System.out.println("After BoardPanel");
		add(side, BorderLayout.EAST);
		System.out.println("After SidePanel add");
		/*
		...
		addKeyListener(new KeyAdapter() {
			...
			public void keyPressed(KeyEvent e) {
				...
				case KeyEvent.VK_S:
					if(!isPaused && dropCooldown == 0) {
						_arch.OUT_IClockType.setCyclesPerSecond(25.0f);
					}
					break;
				 ...
				 */
				case KeyEvent.VK_A:
					if(!isPaused && _arch.OUT_IBoardType.isValidAndEmpty(currentType, currentCol - 1, currentRow, currentRotation)) {
						currentCol--;
					}
					break;
				...
				case KeyEvent.VK_P:
					if(!isGameOver && !isNewGame) {
						isPaused = !isPaused;
						_arch.OUT_IClockType.setPaused(isPaused);
					}
					break;
			...
			public void keyReleased(KeyEvent e) {
				 ...
				 */
				case KeyEvent.VK_S:
					_arch.OUT_IClockType.setCyclesPerSecond(gameSpeed);
					_arch.OUT_IClockType.reset();
				break;
				}
	...
	private void startGame() {
		 ...
		 * to start it.
		 */
		//this._arch.OUT_IClockType =new ClockImp(gameSpeed);
		_arch.OUT_IClockType.setCyclesPerSecond(gameSpeed);
		_arch.OUT_IClockType.setPaused(true);
	...
	private void renderGame() {
		_arch.OUT_IBoardType.boardpanelobject().repaint();
		side.repaint();
	}
	private void updateGame() {
		 ...
		 */
		if(_arch.OUT_IBoardType.isValidAndEmpty(currentType, currentCol, currentRow + 1, currentRotation)) {
			 ...
			 */
			_arch.OUT_IBoardType.addPiece(currentType, currentCol, currentRow, currentRotation);
			 ...
			 */
			int cleared = _arch.OUT_IBoardType.checkLines();
			if(cleared > 0) {
				score += 50 << cleared;
			}
	...
	private void spawnPiece() {
		 ...
		 */
		if(!_arch.OUT_IBoardType.isValidAndEmpty(currentType, currentCol, currentRow, currentRotation)) {
	...
	private void rotatePiece(int newRotation) {
		...
		} else if(currentCol + currentType.getDimension() - right >= IBoardInterface.COL_COUNT) {
			newColumn -= (currentCol + currentType.getDimension() - right) - IBoardInterface.COL_COUNT + 1;
		}
		...
		} else if(currentRow + currentType.getDimension() - bottom >= IBoardInterface.ROW_COUNT) {
			newRow -= (currentRow + currentType.getDimension() - bottom) - IBoardInterface.ROW_COUNT + 1;
		}
		 ...
		 */
		if(_arch.OUT_IBoardType.isValidAndEmpty(currentType, newColumn, newRow, newRotation)) {
